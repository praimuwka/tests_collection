...
+(function ($) {
    ...
    EditableSelect.prototype.select = function ($li) {
        ...
        let tho = this;
        setTimeout(function () {
            tho.$input.parent().find('.editable-select-value').val($li.val());//------------------edited------------
            tho.$input.css('background-color', 'lightgreen');
        }, 75)
    ...
    EditableSelectUtility.prototype.initializeInput = function () {
        ...
        that.es.$input.on('input', function (e) { //---------------------Добавил колбэк--------------------
            that.es.$input.css('background-color', 'white');
            that.es.$input.parent().find('.editable-select-value').val("");
            that.es.$input.parent().find('.es-list li').each(function () {
                if (that.es.$input.val() === $(this).text())
                    that.es.select($(this));
            })
        });
